...
public class MyUSBListAdapter extends BaseAdapter {
    ...
    private static HashMap<Integer, Boolean> isSelected;   // 用来判断CheckBox的选中情况
    ...
    public MyUSBListAdapter(Context context, List<Map<String, Object>> list) {
        ...
        initIsSelectedAndIsFileAdded(list);
    }

    public static void initIsSelectedAndIsFileAdded(List<Map<String, Object>> list) {
        for (int i = 0; i < list.size(); i++) {
            MyUSBListAdapter.getIsSelected().put(i, false);
            MyUSBListAdapter.isFileAdded[i] = false;
        }
    }
